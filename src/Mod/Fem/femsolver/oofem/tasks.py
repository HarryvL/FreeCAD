...
import feminout.importOOFEMResults as importOOFEMResults
...
class Prepare(run.Prepare):
    ...
    def run(self):
        ...
        _inputFileName = os.path.splitext(os.path.basename(path))[0]  # Empty for OOFEM ??
        # print(path)
...
#        self.signalAbort.remove(self._process.terminate)
        ...
        binary = settings.getBinary("oofem")
        self._process = subprocess.Popen(
            [binary, "-c", "-choly"],
            cwd=self.directory,
            stdout=subprocess.PIPE,
...
class Results(run.Results):
    ...
    def load_results_OOFEM(self):
        disp_result_file = os.path.join(
            self.directory, 'OOFEM.txt')
        if os.path.isfile(disp_result_file):
            result_name_prefix = 'OOFEM_' + self.solver.AnalysisType + '_'
            importOOFEMResults.import_OOFEM_disp(
